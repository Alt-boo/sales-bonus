...
function analyzeSalesData(data, options) {
    ...
    if (!data 
        ...
        throw new Error('Некорректные входные данные: отсутствуют или пустые коллекции sellers, products или purchase_records');
    }
    ...
    const { calculateRevenue, calculateBonus } = options;
     if (!calculateRevenue || typeof calculateRevenue !== 'function') {
        throw new Error('Функция calculateRevenue не передана или не является функцией');
    }
    if (!calculateBonus || typeof calculateBonus !== 'function') {
        throw new Error('Функция calculateBonus не передана или не является функцией');
    }
